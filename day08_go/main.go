...
// returns a slice of positions that are antinodes for the two given positions
// but only if they're on the map (so it may return 0, 1, or 2 positions)
func (m *antennaMap) antinodes(p1, p2 position) []position {
	antinodes := []position{}
...
func part1(lines []string) int {
	antennaMap := parseAntennaMap(lines)
	allNodes := map[position]struct{}{}
	count := 0
	for freq, positions := range antennaMap.m {
		fcount := 0
		for i, p1 := range positions {
			for _, p2 := range positions[i+1:] {
				antinodes := antennaMap.antinodes(p1, p2)
				count += len(antinodes)
